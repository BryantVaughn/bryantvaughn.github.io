...
document.addEventListener('DOMContentLoaded', () => {
	...
	function buildOverlay(projectCard) {
		...
		closeBtn.id = 'close';
		addTextContent(closeBtn, 'x');

		const projImg = createElement('img', 'project-img');
		projImg.src = `${projectData.img}`;
		projImg.alt = `${projectData.title} App`;
		...
		const description = addTextContent(
			createElement('p', 'project-desc'),
			`${projectData.desc}`
		);
		const skillsDiv = configureSkills(projectData.tech);
		const visitBtn = configureBtn(
			createElement('a', 'btn btn-sm visit'),
			`${projectData.liveURL}`
		);
		const githubBtn = configureBtn(
			createElement('a', 'btn btn-sm repo'),
			`${projectData.githubURL}`
		);
		addTextContent(visitBtn, 'Visit');
		addTextContent(githubBtn, 'Repo');

		// Append items to overlay
		const projectItems = [
			closeBtn,
			projImg,
			title,
		...
		appendItems(overlayData, projectItems);
		displayOverlay();
	}

	function configureSkills(skills) {
		const skillsDiv = createElement('div', 'skills-icons row');
		skills.forEach((skill) => {
			const skillContainer = createElement('div', 'col-md-4 skill');
			skillContainer.innerHTML += svgs[`${skill}`];
			appendItems(skillsDiv, [skillContainer]);
		});
		return skillsDiv;
	}
	...
	// Event callbacks
	function handleProjectClick(evt) {
